...
class Predictor:
    def __init__(
        self,
        model,
        tile_size: Union[int, Tuple[int, int]] = (500, 500),
        margin: int = 0,
    ...
    def predict(
        self, raster: Raster, batch_size: int = 4, device: str = "cpu"
    ) -> np.array:
        h, w = raster.size
        ...
        x_pos = np.round(
            np.arange(x_step + 1) / x_step * (h - self.tile_size[0])
        ).astype(np.int32)
        y_pos = np.round(
            np.arange(y_step + 1) / y_step * (w - self.tile_size[1])
        ).astype(np.int32)
        ...
        for pos in tqdm(
            batch_items(list(product(x_pos, y_pos)), batch_size),
            total=int(np.ceil((len(y_pos) * len(x_pos)) / batch_size)),
            leave=False,
        ):
            crops = np.stack(
                [
                    pos2crop(image_padded, x, y, self.tile_size, self.margin)
                    for x, y in pos
                ]
            )
            ...
            for idx, (x, y) in enumerate(pos):
                ...
                probs_sum[
                    :, x : x + self.tile_size[0], y : y + self.tile_size[1]
                ] += probs[idx]
    ...
    def compute_step(size: int, tile_size: int, margin: int, min_overlap: float) -> int:
        return np.ceil(
            (size - tile_size - margin) / ((tile_size - margin) * (1 - min_overlap))
        )
...
def imgs2preds(imgs, model, device="cpu", margin=16) -> torch.Tensor:
    ...
    with torch.no_grad():
        preds = (
            torch.sigmoid(model.to(device)(imgs_torch))
            .cpu()
            .numpy()[:, :, margin:-margin, margin:-margin]
        )
    return preds
